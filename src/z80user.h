 ...
 * the end of the file for an example for running vzcontext.
 *
...
#define Z80_FETCH_BYTE(address, x)		{(x) = emu_fetch_byte(context, address);}

#define Z80_READ_BYTE(address, x)       {(x) = emu_read_byte(context, address);}
#define Z80_READ_WORD(address, x)       {(x) = emu_read_word(context, address);}


#define Z80_WRITE_BYTE(address, x)      {emu_write_byte(context, address, x);}
#define Z80_WRITE_WORD(address, x)      {emu_write_word(context, address, x);}
