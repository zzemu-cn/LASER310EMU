 ...
 *
 * Copyright (c) 2012-2020 Lin Ke-Fong, zzemu
 *
...
int Z80Interrupt (Z80_STATE *state, uint8_t data_on_bus, void *context)
{
	state->status = 0;
	if (state->iff1) {
...
/*
int Z80Emulate (Z80_STATE *state, void *context)
{
	int  pc, opcode;
	state->status = 0;
	pc = state->pc;
	...
	state->pc = pc + 1;
	return emulate(state, opcode, context);
}
*/
 ...
 */
int z80emulate (Z80_STATE * state,
	uint8_t opcode,
	void *context)
{
	uint16_t	pc;
	...
	//INFO("pc %04X BC %04X", state->pc, state->registers.word[Z80_BC]);
	...
	if(opcode) {
		INFO("RSTN pc %04X %02X", pc, opcode);
...
start_emulation:
		...
		// DD_PREFIX ED_PREFIX FD_PREFIX 换了 registers

start_decode:
		// DD_PREFIX FD_PREFIX
		instruction = INSTRUCTION_TABLE[opcode];
		// ED_PREFIX 换了 instruction
...
start_instruction:
		...
		switch (instruction) {
			...
			case LDIR_LDDR: {
				...
				//printf("d %d opcode %02x elapsed_cycles %d bc %d pc %04x\n", d, opcode, elapsed_cycles, bc, pc);
				f = F & SZC_FLAGS;
				bc = BC;
				de = DE;
				hl = HL;
				...
				elapsed_cycles -= 8;
				for ( ; ; ) {
					//printf("opcode %02x elapsed_cycles %d bc %d pc %04x\n", opcode, elapsed_cycles, bc, pc);
					r += 2;
					Z80_READ_BYTE(hl, n);
					Z80_WRITE_BYTE(de, n);
					INFO("%04X:%02X => %04X bc %04X pc %04X", hl, n, de, bc, pc);
					hl += d;
					de += d;
					if (--bc)
							elapsed_cycles += 21;
...
next_emulation:
	//state->pc = pc & 0xffff;
	state->pc = pc;
	//INFO("next pc %04X elapsed_cycles %d", state->pc, elapsed_cycles);
